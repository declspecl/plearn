        ...
        return (
            <SidebarInstanceContext.Provider value={instanceContextValue}>
                <Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
                    <SheetPopup
                        className={cn("bg-sidebar text-sidebar-foreground w-(--sidebar-width) max-w-none p-0", className)}
                        data-mobile="true"
                    ...
                    >
                        ...
                        <div className={cn("pb-safe pt-safe flex h-full w-full flex-col", side === "left" ? "pl-safe" : "pr-safe")}>
                            {children}
                        </div>
                    </SheetPopup>
                </Sheet>
...
function SidebarRail({
    ...
    const handlePointerMove = React.useCallback(
        (event: React.PointerEvent<HTMLButtonElement>) => {
            ...
            event.preventDefault();
            const delta = resizeState.side === "right" ? resizeState.startX - event.clientX : event.clientX - resizeState.startX;
            if (Math.abs(delta) > 2) {
    ...
    return (
        <button
            ...
            className={cn(
                "hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex [[data-collapsible=offcanvas][data-state=collapsed]_&]:pointer-events-none",
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
    return (
        <ScrollArea hideScrollbars scrollFade className="h-auto min-h-0 flex-1">
            <div
                className={cn("flex w-full min-w-0 flex-col gap-2 group-data-[collapsible=icon]:overflow-hidden", className)}
