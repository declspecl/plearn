    ...
    const defaultProps = {
        children: (
            <>
                ...
                {variant === "ghost" ? <CaretDown className="size-3 opacity-50" /> : <CaretUpDown className={selectTriggerIconClassName} />}
            </>
        ),
...
function SelectPopup({
    ...
    return (
        <SelectPrimitive.Portal>
            ...
            >
                <SelectPrimitive.Popup className="text-foreground origin-(--transform-origin)" data-slot="select-popup" {...props}>
...
function SelectItem({
    ...
    return (
        <SelectPrimitive.Item
            className={cn(
                "data-highlighted:bg-accent data-highlighted:text-accent-foreground grid min-h-8 cursor-default items-center gap-2 rounded-sm py-1 text-base outline-none in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] data-disabled:pointer-events-none data-disabled:opacity-64 sm:min-h-7 sm:text-sm [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4",
        ...
        >
            ...
            <SelectPrimitive.ItemText className={cn("min-w-0", hideIndicator ? "col-start-1" : "col-start-2")} data-slot="select-item-text">
                {children}
...
function SelectSeparator({ className, ...props }: SelectPrimitive.Separator.Props) {
    return <SelectPrimitive.Separator className={cn("bg-border mx-2 my-1 h-px", className)} data-slot="select-separator" {...props} />;
}
